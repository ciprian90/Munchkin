...
/**
 * WebMunchkin Login Server. Nimmt SSL (Secure Sockets Layer) gesicherte
 * Verbindungen von WebMunchkinClients an.
 * 
...
public class Server
{
	...
	// Klassenvariablen
	static Connection sqlConnection = null; // Speichert die SQL Datenbank
											// Verbindung
	 ...
	 * 
	 * 
	 private static void test() { try { sqlStatement =
	 * sqlConnection.createStatement(); if (sqlStatement.execute(
	 * "SELECT * FROM `account` WHERE `login` = 'deadjack' LIMIT 0 , 30")) {
	 * sqlResult = sqlStatement.getResultSet();
	 * Utility.debugMsg(sqlResult.toString()); if (sqlResult.next()) {
	 * Utility.debugMsg(sqlResult.getNString("username")); } } } catch
	 * (SQLException e) { e.printStackTrace(); }finally { // Alle gebundenen
	 * Ressourcen wieder releasen (bzgl. MySQL Datenbank) // Release statement
	 * if (sqlStatement != null) { try { sqlStatement.close(); } catch
	 * (SQLException sqlEx) { } // ignore sqlStatement = null; } // Release
	 * result if (sqlResult != null) { try { sqlResult.close(); } catch
	 * (SQLException sqlEx) { } // ignore sqlResult = null; } } }
	 */
	...
	private static void connectToSqlDatabase()
	{
		...
		{
			Utility.errorMsg(
					"Could not establish connection to sql database. SQL-State: "
							+ e.getSQLState()
							+ "\nCheck http://www.postgresql.org/docs/8.2/static/errcodes-appendix.html",
					e);
		}
	...
	private static void serverTasks()
	{
		...
		try
		{
			sslserversocket = (SSLServerSocket) sslserversocketfactory
					.createServerSocket(Constants.port);
		}
		catch (IOException e)
		{
		...
		while (true)
		{
			try
			{
				...
				Thread tempThread = new Thread(new ServerInstance(sslsocket,
						sqlConnection));
			...
			{
				// Verwerfe diese Verbindung, da sie nicht aufgebaut werden
				// konnte
